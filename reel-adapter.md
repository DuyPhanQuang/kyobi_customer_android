...
Tối ưu chuyển nguồn: Sử dụng ConcatenatingMediaSource2 để chuyển nguồn từ remote sang local mượt mà hơn.
...
switchToLocalIfAvailable: Chuyển nguồn từ remote sang local khi file đã tải xong.
...
Tải trước file:downloadNextVideoPartial kiểm tra và tải trước file cho position + 1 và position + 2 nếu chưa tải:
...
Hủy tải không cần thiết:Các position không nằm trong positionsToKeep (khoảng ±3 so với position hiện tại) sẽ bị hủy tải để tiết kiệm tài nguyên:
...
Tốn bộ nhớ: Mỗi file tải về chiếm khoảng 5MB. Với 7 file (position -3 đến +3), bộ nhớ tối đa là ~35MB, vẫn trong ngưỡng chấp nhận được.
...
Giới hạn activePlayers:Các player ngoài khoảng cách ±10 so với position hiện tại sẽ bị xóa:
val maxDistance = 10
...
Giới hạn downloadedFiles:Các file ngoài khoảng cách ±3 so với position hiện tại sẽ bị xóa thông qua positionsToKeep trong downloadNextVideoPartial.
...
4. Chuyển Nguồn Từ Remote Sang Local
   Mô tả
   Để giảm độ trễ và tiết kiệm băng thông, video ban đầu được phát từ URL remote, sau đó chuyển sang phát từ file local (5MB đầu tiên) khi file tải xong. Việc chuyển nguồn được tối ưu bằng ConcatenatingMediaSource2 để giảm giật lag.
   Giải pháp hiện tại

Kiểm tra và chuyển nguồn:Trong playVideoAtPosition, sau khi phát video từ remote, ứng dụng kiểm tra xem file local đã tải xong chưa. Nếu có, chuyển nguồn sang local:
...
Tải song song:Nếu file chưa tải, downloadVideoPartial được gọi để tải file, và sau khi tải xong, nguồn sẽ được chuyển:
downloadVideoPartial(position) {
mainHandler.post {
switchToLocalIfAvailable()
}
...
Tối ưu chuyển nguồn với ConcatenatingMediaSource2:Sử dụng ConcatenatingMediaSource2 để chuyển nguồn mượt mà hơn:
...
Ban đầu, video phát từ remote:ReelAdapter: Creating remote media source for position: 0
ReelAdapter: Did not switch to local file at position 0: containsKey=false, isPlaying=false


Sau khi file tải xong (sau ~2 giây), nguồn chuyển sang local:ReelAdapter: Successfully downloaded first 5MB for position 0 (2463092 bytes)
ReelAdapter: Switched to local file at position: 0 localFile: /data/user/0/com.kyobi.customer/cache/video_0_partial.mp4



Scroll nhanh từ position 0 → 5:

Tại position 5, nếu file chưa tải xong, video phát từ remote trước:ReelAdapter: Creating remote media source for position: 5
...
ReelAdapter: Switched to local file at position: 5



Phân tích rủi ro

Thời gian tải file: Nếu mạng chậm, việc tải file có thể mất vài giây (trong log, position 0 mất ~2 giây). Trong thời gian này, video phát từ remote, có thể gây giật nếu mạng không ổn định.
...
Giật lag khi chuyển nguồn: ConcatenatingMediaSource2 được sử dụng để giảm giật lag khi chuyển nguồn. Tuy nhiên, cần kiểm tra trên thiết bị thực tế để đảm bảo không có giật lag, đặc biệt khi video đang phát.
...
Tràn bộ nhớ: Với giới hạn 21 player và 7 file (~35MB), nguy cơ tràn bộ nhớ đã được kiểm soát tốt trên các thiết bị thông thường (có bộ nhớ từ 2GB trở lên). Tuy nhiên, trên thiết bị yếu (bộ nhớ < 1GB), việc giữ 21 player có thể gây áp lực.
Đề xuất: Giảm maxDistance xuống 5 (tối đa 11 player) nếu cần.


Hiệu suất: Việc khởi tạo và giải phóng ExoPlayer liên tục khi scroll nhanh có thể gây chậm nhẹ, nhưng không đáng kể nhờ giới hạn activePlayers.
